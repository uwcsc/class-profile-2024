...
export default function WindowPanel({ dark = false, blank = false, children }: PropsWithChildren<{ dark?: boolean; blank?: boolean }>) {
  if (blank) return <div className="p-4 md:p-8 w-[calc(min(80vw,1200px))]">{children}</div>;

  return (
    <div className="flex flex-col items-center">
      <div
        className={`${dark ? "bg-window-panel-title-dark" : "bg-window-panel-title-light"} px-5 py-3 rounded-t-xl flex gap-3 w-[calc(min(80vw,1200px))] backdrop-blur-sm`}>
      ...
      <div className="bg-window-panel-background p-4 md:p-8 rounded-b-xl w-[calc(min(80vw,1200px))] backdrop-blur-sm">{children}</div>
    </div>
  );
}
