...
export default function WindowPanel({ dark = false, blank = false, children }: PropsWithChildren<{ dark?: boolean; blank?: boolean }>) {
  if (blank) return <div className="p-4 md:p-8 w-[calc(min(75%,1000px))]">{children}</div>;
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      <div className="bg-window-panel-background p-4 md:p-8 rounded-b-xl w-[calc(min(90%,1200px))] backdrop-blur-sm">{children}</div>
    </div>
  );
}
